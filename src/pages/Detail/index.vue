      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.lazy="skuNum" />
...
export default {
  ...
  mounted() {
    //派发action获取产品信息
    //开启命名空间后不能简写成this.$store.dispatch('goodsInfo',this.$route.query.id)
    this.$store.dispatch("detail/goodsInfo", this.$route.query.id);
  },
  computed: {
  ...
  methods: {
    ...
    async addShopCart() {
      ...
      //2:请求成功则跳转，失败则提示
      try {
        await this.$store.dispatch("detail/addOrUpdateShopCart", {skuId: this.$route.query.id,skuNum: this.skuNum,})
        //路由跳转，并且传参给下一级路由组件
        //skuInfo比较长不用路由传参，通过会话储存
        //JSON.stringify吧JSON对象转换成JSON字符串（sessionStorage只储存字符串）
  ...
  watch: {
    skuNum(val) {
      if (isNaN(val) || val < 1) {
        //非数字或者是负数
        //console.log('我是if,val类型是',typeof val)
        this.skuNum = 1;
      } else {
        //疑问，这里为什么val的类型还是字符串
        //如果是字符串怎么还走else
        //console.log('我是else,val类型是',typeof val)
        //转换成数字并取整
